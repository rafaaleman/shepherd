...
/**
 * [returns an array with the configuration for the creation of the avatar]
 *
 * @return [array]
 * 
 */
 ...
 */
    ...
    */
    ...
    /**
     * [this function creates a png image with the initials of the user, receives an object of type user and a boolean that indicates if the name of the image should be saved in the database]
     *
     * @param User $user
     * @param null $update
     * 
     ...
     * 
     */
    function create_avatar(User $user,$update = null)
    {
        $user->photo = '/members/'.$user->id.'.png';
        $avatar = new Avatar(configAvatar());
        $avatar->create($user->name." ".$user->lastname)->save(public_path('members/'.$user->id.'.png'), $quality = 90);
        if($update){
            $user->save();
        }
        return $user->photo;
